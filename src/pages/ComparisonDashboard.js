...
const ComparisonDashboard = () => {
  ...
  const getFinalDecision = async () => {
    try {
      ...
      const userOfferResponse = await fetch('http://localhost:8000/api/user-decision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          scenario, 
      ...
      const lawyerDecisionResponse = await fetch('http://localhost:8000/api/lawyer-decision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          scenario,
